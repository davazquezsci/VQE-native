...
#   1) curvas FCI y Hartree--Fock de referencia,
...
#   5) región usada para cada ajuste parabólico,
#   6) mínimo estimado por cada parábola.
#
# Además genera un archivo CSV con los mínimos estimados para cada
# curva VQE y sus errores respecto a FCI.
#
# Importante:
#   - Se grafica la región usada para el ajuste parabólico.
...
# Región local usada para cada ajuste parabólico.
MOSTRAR_REGION_AJUSTE = True

# Marcadores para el mínimo estimado por cada parábola.
MOSTRAR_MINIMOS = True
...
ALPHA_REGION_AJUSTE = 0.08
LINEWIDTH_REGION_AJUSTE = 0.75
...
def leer_csv_curva(config: dict) -> pd.DataFrame:
    ...
    columnas_necesarias = [COL_R, COL_VQE, COL_FCI, COL_HF]
...
def graficar():
    ...
    E_hf_ref = df_ref[COL_HF].to_numpy(dtype=float)
    ...
    for i, res in enumerate(resultados):
        ...
        # Región usada para el ajuste parabólico de esta curva.
        if MOSTRAR_REGION_AJUSTE:
            etiqueta_region = r"Región del ajuste parabólico" if i == 0 else None
            ax.axvspan(
                res["R_region_ajuste_min"],
                res["R_region_ajuste_max"],
                color=color,
                alpha=ALPHA_REGION_AJUSTE,
                linewidth=0,
                label=etiqueta_region,
                zorder=1 + i,
            )
            ax.axvline(
                res["R_region_ajuste_min"],
                ...
                alpha=0.45,
                linestyle=":",
                linewidth=LINEWIDTH_REGION_AJUSTE,
                zorder=2 + i,
            )
            ax.axvline(
                res["R_region_ajuste_max"],
        ...
        if MOSTRAR_MINIMOS:
            ax.scatter(
                ...
                marker="D",
                color=color,
                edgecolors=COLOR_MINIMO,
                alpha=1,
                linewidths=0.45,
                label=rf"Mínimo parabólico, {cfg['shots']} shots",
                zorder=20 + i,
            )
